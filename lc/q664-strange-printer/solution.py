class Solution:
    def strangePrinter(self, s: str) -> int:
        ...
        # Create an n x n matrix to store the minimum number of turns needed to print s[i: j+1] as value dp[i][j]
        ...
        for i in range(n - 1, -1, -1):
            ...
            for j in range(i + 1, n):
                # If s[i] and s[j] are different characters, we try to use the previously computed dp
                # values and update the current dp value by taking the minimum value of the two
                # possible options
                if s[i] == s[j]:
                    dp[i][j] = dp[i][j - 1]
